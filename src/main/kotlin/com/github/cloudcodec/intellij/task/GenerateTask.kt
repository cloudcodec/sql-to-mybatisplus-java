...
class GenerateTask(
    ...
    private fun getPackageName(directory: VirtualFile): String {
        ...
        while (cur != null) {
            if (cur.name == lang && cur.parent != null && cur.parent.name == "main" && cur.parent.parent != null && cur.parent.parent.name == "src") {
                if (nameList.isEmpty()) {
                    throw IllegalStateException("there is no package in src/main/${lang}, please create first, director: ${directory.path}")
                }
        ...
        throw IllegalStateException("can't find src/main/$lang in ${directory.path}")
    }
